...
def main(fred, fblue, param0, outname, mode='SP', inc=45., dist=140., vsys=None, paramerr_figout=True, offthr_red_in=None, offthr_blue_in=None, offthr_red_out=None, offthr_blue_out=None):
    ...
    if mode == 'SP':
        ...
        print ('V100 (LSRK): %.2f km/s'%param_SPout[0])
        print ('power-law index: %.2f'%param_SPout[1])
        print ('chi-square')
        print ('degree of freedum: %i'%DOF)
        print ('reduced chi-square: %.2f'%reduced_chi2)
        ...
        print ('v100 \t  p')
        print ('%.2f,%.2f'%(v100sig, psig))
        print ('%.2f,%.2f'%(v100median, pmedian))
        ...
        print ('output parameters')
        #print 'chi2', chi2
        print ('Vbreak: %.2f km/s'%(param_out[0]))
        print ('Rbreak: %.2f au'%(param_out[1]))
        print ('p at inner region: %.4f'%(param_out[2]))
        print ('p at outer region: %.4f'%(param_out[3]))
        print ('chi-square')
        ...
        # result
        radius = np.array([r for r in range(1,1200)])
        ...
        print ('%.2f %.2f %.4f %.4f'%(vbsig, rbsig, pinsig, poutsig))
    ...
    ax.set_xlim(20,200)
    ax.set_ylim(0.5,5)
    ax.set_xlabel(r'$\mathrm{radius\ (au)}$')
    ax.set_ylabel(r'$\mathrm{velocity\ (km\ s^{-1})}$')
...
if __name__ == '__main__':
    ...
    rms       = 0.14 # Jy/beam, for taper 500k

    fred  = 'l1489_c18o21_model_vff_red_chi2_pv_vfit.txt'
    fblue = 'l1489_c18o21_model_vff_blue_chi2_pv_vfit.txt'
    inc   = 73.  # degree
    vsys  = 7.22  # km/s
    dist  = 140. # pc
    offthr_red_in  = 10. # arcsec
    offthr_red_out = None
    offthr_blue_in = 10. # arcsec
    offthr_blue_out = None
    Mstar = 1.6*Msun
    rc    = 600. # au
    jang  = np.sqrt(rc*auTOcm*Ggrav*Mstar)*1e-5/pcTOcm
    print (jang)


    outname   = 'rot_plawfit_result_54d_DP'
    mode      = 'DP'
    param0_sp = [7.3, 3., 0.5]
    param0_dp = [1.2,  3000., 1, -0.2] # for DP

    ax = main(fred, fblue, param0_dp, outname, mode=mode, inc=inc, dist=dist, vsys=vsys, offthr_red_in=offthr_red_in, offthr_blue_in=offthr_blue_in, offthr_red_out=offthr_red_out, offthr_blue_out=offthr_blue_out)
    ax.set_xlim(650, 6500)
    ax.set_ylim(0.16, 1.6)
    ax.set_aspect(1)
    ...
    ax.plot(r/auTOcm, jang*(r/auTOcm*auTOpc)**(-1.), color='r', ls='-', lw=1)

    # ticks
    # y
    ax.set_yticks([1],minor=False)
    ax.set_yticks(np.arange(0.2,0.9,0.1),minor=True)
    ax.set_yticklabels(['1'], minor=False)
    ax.set_yticklabels(['0.2','','','','0.6'], minor=True)
    # x
    ax.set_xticks([1000],minor=False)
    ax.set_xticks([700,800,900, 2000, 3000, 4000, 5000, 6000],minor=True)
    ax.set_xticklabels(['','','','','3000', '', '5000'], minor=True)
