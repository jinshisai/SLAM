...
if __name__ == '__main__':
    ...
    outname   = 'rot_plawfit_result_54d_DP_check_vsys'
    mode      = 'DP'
    param0_dp = [1.2,  500., 0.5, 1.] # for DP

    list_vsys = np.arange(7.18,7.26,0.01)


    fig = plt.figure(figsize=(11.69,8.27))
    ax = fig.add_subplot(111)

    for vsys in list_vsys:
        print 'vsys: ', vsys, ' km/s'
        chi2_vsys = main(fred, fblue, param0_dp, outname, mode=mode, inc=inc, dist=dist, vsys=vsys, offthr_red_in=offthr_red_in, offthr_blue_in=offthr_blue_in, offthr_red_out=offthr_red_out, offthr_blue_out=offthr_blue_out)
        ax.scatter(vsys,chi2_vsys,c='darkblue',marker='o',s=40)


    ax.tick_params(which='both', direction='in',bottom=True, top=True, left=True, right=True, pad=9)
    ax.set_xlabel(r'$V_\mathrm{sys}$ (km/s)')
    ax.set_ylabel(r'reduced $\chi^2$')
    pyfg.change_aspect_ratio(ax,1.)
