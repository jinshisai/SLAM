...
def main(fred, fblue, param0, outname, mode='SP', inc=45., dist=140., vsys=None, paramerr_figout=True, offthr=None):
    '''
    ...
    if mode == 'SP':
        ...
        chi2         = sum(chiSPlaw(param_SPout, offset, velocity, offerr, velerr)**2.)
        ndata        = len(offset)
        ...
        v100   = param_SPout[1] * 1.e5 # km/s --> cm/s
        ...
        for n in xrange(3000):
            offest = norm.rvs(size = len(offset), loc = offset, scale = offerr)
            velest = uniform.rvs(size = len(velocity), loc = velocity, scale = velerr) # assuming uniform distribution because the error of V is velocity resolution
        ...
        # error of stellar mass
        dMstar = Mstar*2.*v100sig/param_SPout[1]
        print 'dMstar: ', dMstar, ' Msun'
        print 'all of errors mean 1 sigma'
...
if __name__ == '__main__':
    ...
    vsys  = 7.3 # km/s
    dist  = 140. # pc
    outname   = 'rot_plawfit_result_69d'
    mode      = 'SP'
    offthr    = 100./dist # from Aso et al. (2015)
    param0_sp = [7.3, 3., 0.5]
    param0_dp = [7.3, 1.2, 650., 0.5, 1.] # for DP

    ax = main(fred, fblue, param0_sp, outname, mode=mode, inc=inc, dist=dist, vsys=vsys, offthr=offthr)
    ax.set_xlim(20,200)
    ax.set_ylim(1,10)

    # ticks
    # y
    ax.set_yticks([1,2,4,6,8,10],minor=False)
    ax.set_yticks([2,3,4,5,6,7,8,9],minor=True)
    ax.set_yticklabels(['1','2','4','6','8','10'], minor=False)
    ax.set_yticklabels([''], minor=True)
    # x
    ax.set_xticks([20,40,60,80,100,200],minor=False)
    ax.set_xticks([20,30,40,50,60,70,80,90,200],minor=True)
    ax.set_xticklabels([''], minor=True)
    ax.set_xticklabels(['20','40','60','80','100','200'], minor=False)
