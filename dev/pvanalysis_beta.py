...
cutoff   = 5.0  # sigma
quadrant = None  # Emitting quadrants, '13' or '24'. None means automatic.
...
def gauss(x, peak, center, hwhm):
    return peak * np.exp2(-(x - center)**2 / hwhm**2)
...
#    def __init__(self):
    ...
    def get_edgeridge(self, pvfits=None, dist=None, vsys=None, sigma=None,
        ...
        def get_points(mode):
            ...
            for tt, dd in zip(t, d_input):
                ...
                grad = (np.roll(d0, -1) - np.roll(d0, 1)) \
                       / (si_org[2] - si_org[0])
                if mode == 'x':
                    M = kepler_mass(si_org, tt, self.__unit)
                elif mode == 'v':
                    M = kepler_mass(tt, si_org, self.__unit)
                cond = (d0 > thre) * between(M, Mlim) * (si_org * tt_s > 0)
                grad, ss, val, err = grad[cond], si_org[cond], np.nan, 0
                if len(ss) > 0:
                    val = ss[0] if tt_s < 0 else ss[-1]
                    grad = grad[0] if tt_s < 0 else grad[-1]
                    err = sigma / np.abs(grad)
                ...
                if interp_ridge:
                    ...
                    d1, ss = dd, s
                cond = (d1 > thre)
                d1, ss, val, err = d1[cond], ss[cond], np.nan, 0
                if ridge == 'mean':
                    if len(ss) > 1:
                        val = np.sum(ss * d1) / np.sum(d1)
                        err = sigma * np.abs(np.sum(ss - val)) / np.sum(d1)
                elif ridge == 'gauss':
                    if len(ss) > 3:
                        bnd = [[0           , -slim[1], ds     ],
                               [d1.max() * 2,  slim[1], slim[1]]]
                        try:
                            popt, pcov = curve_fit(gauss, ss, d1,
                                                   sigma=ss * 0 + sigma,
                                                   absolute_sigma=True,
                                                   bounds=bnd)
                            val, err = popt[1], np.sqrt(np.diag(pcov)[1])
                        except RuntimeError:
                            pass
                err = clipped_error(err, val, mode=mode)
                if not between(val * tt_s, slim): val = np.nan
    ...
    def fit_edgeridge(self, include_vsys=False, include_dp=True,
        ...
        plim = np.array([[0, 0, 0.01, 0, -1], [1000, 100, 10, 10, 1]])
        if not include_pin:  plim[0][2], plim[1][2] = 0.5 - 1e-6, 0.5 + 1e-6
        if not include_dp:   plim[0][3], plim[1][3] = -1e-6, 1e-6
        if not include_vsys: plim[0][4], plim[1][4] = -1e-6, 1e-6
        
        def lnprob(p, v0, x1, dx1, x0, v1, dv1):
            if np.all(between(p, plim)):
                chi2 = np.sum(((x1 - doublepower_r(v0, *p)) / dx1)**2) \
                       + np.sum(((v1 - doublepower_v(x0, *p)) / dv1)**2)
                return -0.5 * chi2
            else:
                return -np.inf
        
        def mcmcfit(args, ext):
            plim[0][0] = np.min(np.abs(np.r_[args[1], args[3]]))
            plim[0][1] = np.min(np.abs(np.r_[args[0], args[4]]))
            plim[1][0] = np.max(np.abs(np.r_[args[1], args[3]]))
            plim[1][1] = np.max(np.abs(np.r_[args[0], args[4]]))
            if not include_dp:
                plim[0][1] = np.sqrt(plim[0][1] * plim[1][1]) - 1e-6
                plim[1][1] = plim[0][1] + 2e-6
            ndim = len(plim[0])
            nwalkers = ndim * 16
            nburnin = 1000
            nsteps = 2000
            ...
            #!!!!! emcee 3.1.1 !!!!!#
            #state = sampler.run_mcmc(p0, nburnin + nsteps)
            #samples = sampler.get_chain()[:, nburnin:, :].reshape((-1, ndim))
            #lnp = sampler.get_log_prob()
            #chain = sampler.get_chain()
            #popt = chain[np.unravel_index(np.argmax(lnp), lnp.shape)]
            #!!!!!!!!!!!!!!!!!!!!!!!#
            #!!!!! emcee 2.2.1 !!!!!#
            state = sampler.run_mcmc(p0, nburnin + nsteps)
            samples = sampler.chain[:, nburnin:, :].reshape((-1, ndim))
            lnp = sampler.lnprobability
            popt = sampler.chain[np.unravel_index(np.argmax(lnp), lnp.shape)]
            #!!!!!!!!!!!!!!!!!!!!!!!#
            plow = np.percentile(samples, 16, axis=0)
            pmid = np.percentile(samples, 50, axis=0)
            phigh = np.percentile(samples, 84, axis=0)
            perr = (phigh - plow) / 2.
            rng = np.array([0.8, 0.8, 0.8, 0.8, 0.8])
            labels = np.array(['Rb', 'Vb', 'p_in', 'dp', 'Vsys'])
            include = [True, include_dp, include_pin, include_dp, include_vsys]
            corner.corner(samples[:, include], truths=popt[include],
                          quantiles=[0.16, 0.5, 0.84], show_titles=True,
                          range=rng[include], labels=labels[include]),
            plt.savefig(filehead + '.corner' + ext + '.png')
        ...
        popt_e, perr_e = mcmcfit(Es, '_e')
        popt_r, perr_r = mcmcfit(Rs, '_r')
        print(f'- Plotted in {filehead}.corner_e.png '
              + f'and {filehead}.corner_r.png')
        self.popt = {'edge':[popt_e, perr_e], 'ridge':[popt_r, perr_r]}
        result = {'edge':{'popt':popt_e, 'perr':perr_e},
                  'ridge':{'popt':popt_r, 'perr':perr_r}}
        return result
