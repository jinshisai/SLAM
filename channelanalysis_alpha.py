...
############ INPUTS ############
cubefits = '/scratch/alma/yaso/home/eDisk/TMC1A/TMC1A_C18O_t2000klam.image.common.fits'
center = '04h39m35.202290s 25d41m44.22436s',
pa = 75  # deg
incl = 48  # deg
vsys = 6.4  # km/s
dist = 140  # pc
...
cutoff = 5.0  # sigma
tol_kep = 0.5  # beam; tolerance from the major axis
xmax = 200.  # au
ymax = xmax  # au
vmax = 6  # km/s
vmin = 0  # km/s
...
#    def __init__(self):
    ...
    def find_rkep(self, pa=0., tol_kep=0.5):
        ...
        k_shift = [np.where(sc[i] > tol)[0][-1] for i in [0, 1]]
        k_notkep = [max(k_back[i], k_shift[i]) for i in [0, 1]]
        self.k_notkep = k_notkep
        for i, c in zip([0, 1], ['red', 'blue']):
            if max(k_back[i], k_shift[i]) == len(sc[i]) - 1:
                print('!!!No Keplerian point on the ' + c + 'shifted side.!!!')
    ...
    def plot_center(self, filehead='channelanalysis', show_figs=False,
        ...
        ax.set_xlim(-tol * 3, xmax * 1.001)  # au
        ax.set_ylim(-tol * 3, ymax * 1.001)  # au
        ax.grid()
        fig.tight_layout()
        ...
        ax.scatter(t_n, np.abs(v_n), s=100, c=s_n / self.bmaj,
                   marker='^', cmap='cubehelix', vmin=0, vmax=1.5, zorder=2)
        sca = ax.scatter(t_k, np.abs(v_k), s=100, c=s_k / self.bmaj,
                         marker='o', cmap='cubehelix',
                         vmin=0, vmax=2 * self.tol_kep, zorder=3)
        cb = plt.colorbar(sca, ax=ax, label='Minor offset (beam)')
...
if __name__ == '__main__':
    filehead = cubefits.split('/')[-1].replace('.fits', '')
    chan = ChannelAnalysis()
    chan.get_2Dcenter(cubefits=cubefits, center=center,
    ...
    chan.find_rkep(pa=pa, tol_kep=tol_kep)
