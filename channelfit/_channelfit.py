...
#    def __init__(self):
    ...
    def gridondisk(self, cubefits: str = None,
        ...
        def cubemodel(logMstar: float, logRc: float, logcs: float,
            ...
            m = np.where(self.mom0 < 3 * self.sigma_mom0, 0,
    ...
    def fitting(self, Mstar_range: list = [0.01, 10],
        ...
        if None in p_fixed:
            ...
            self.pmid = p_fixed
            self.phigh = p_fixed
        print('plow:', ', '.joint([f'{t:.2e}' for t in self.plow]))
        print('pmid:', ', '.joint([f'{t:.2e}' for t in self.pmid]))
        print('phigh:', ', '.joint([f'{t:.2e}' for t in self.phigh]))
        print('------------------------')
        print('popt:', ', '.joint([f'{t:.2e}' for t in self.popt]))
        print('------------------------')
